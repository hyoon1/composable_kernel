...
{{
    ...
    if(s.log_level_ > 0)
        std::cout << ", {F_kname}"
                  << " fast_exp2:" << CK_TILE_FMHA_FWD_FAST_EXP2
                  << " bf16_cvt:" << CK_TILE_FLOAT_TO_BFLOAT16_DEFAULT
                  << std::flush;
...
int get_forced_m0() {
    const char* env = std::getenv("CK_TILE_FMHA_FWD_FORCE_M0");
...
int get_forced_flag(const char* name) {
    const char* env = std::getenv(name);
    if(env == nullptr || env[0] == '\\0') {
        return -1;
    }
    return std::atoi(env) ? 1 : 0;
}
} // namespace
"""
...
float {F_func_name}([[maybe_unused]] fmha_fwd_traits t, [[maybe_unused]] fmha_fwd_args a, [[maybe_unused]] const ck_tile::stream_config& s) {{
    ...
    [[maybe_unused]] const int forced_m0 = get_forced_m0();
    [[maybe_unused]] const int forced_spad = get_forced_flag("CK_TILE_FMHA_FWD_FORCE_SPAD");
    [[maybe_unused]] const int forced_skpad = get_forced_flag("CK_TILE_FMHA_FWD_FORCE_SKPAD");
    [[maybe_unused]] const int forced_dpad = get_forced_flag("CK_TILE_FMHA_FWD_FORCE_DPAD");
    [[maybe_unused]] const int forced_dvpad = get_forced_flag("CK_TILE_FMHA_FWD_FORCE_DVPAD");
...
FMHA_FWD_API_INNER_DISPATCH = """{F_if}(((forced_m0 < 0) || ({F_bm0} == forced_m0)) &&
        ((forced_spad < 0) || (static_cast<int>({F_spad}) == forced_spad)) &&
        ((forced_skpad < 0) || (static_cast<int>({F_skpad}) == forced_skpad)) &&
        ((forced_dpad < 0) || (static_cast<int>({F_dpad}) == forced_dpad)) &&
        ((forced_dvpad < 0) || (static_cast<int>({F_dvpad}) == forced_dvpad)) &&
        ...
        ({F_scheck}) && ((forced_m0 >= 0) || ({F_seqtune})) && ({F_skcheck}) && ({F_dcheck}) && ({F_dvcheck}) && ({F_constraint})) {{
